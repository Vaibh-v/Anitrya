...
export type BehaviorEvidenceSummary = {
  landingRows: number;
  sourceRows: number;
};
...
export async function getBehaviorEvidenceSummary(input: {
  workspaceId: string;
  projectId: string;
  from: string;
  to: string;
}): Promise<BehaviorEvidenceSummary> {
  const [landingRows, sourceRows] = await Promise.all([
    countRows({
